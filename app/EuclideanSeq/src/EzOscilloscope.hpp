...
private:
    ...
    byte readDataLong()
    {
        ...
        // _dataBuff[index] = _pCv->analogReadDirect();
        index++;
        // 線描画の終端の関係で+1
    ...
    void readData()
    {
        for (byte i = 0; i < DATA_BUF_MAX; ++i)
        {
            // _dataBuff[i] = _pCv->analogRead(true);
            _dataBuff[i] = _pCv->analogRead(false);
            // _dataBuff[i] = _pCv->analogReadDirect();
    ...
    void drawString()
    {
        ...
        _pU8g2->setFont(u8g2_font_7x14B_tf);
        sprintf(chrBuff, "CVMONITOR tm:%5d", _delay);
        _pU8g2->drawStr(0, 2, chrBuff);

        _pU8g2->setFont(u8g2_font_5x8_tf);
        // tmp = _dataAve * _convertVoltCoff;
        // sprintf(chrBuff, "%4.1f", tmp);
